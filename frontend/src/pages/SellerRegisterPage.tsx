...
import { User, Mail, Lock, ShieldCheck, Store, AlertCircle } from 'lucide-react';
...
export const SellerRegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await registerSeller(name.trim(), email, password, storeName.trim());
      if (user.role === 'SELLER') {
    ...
    <div className="register-page">
      ...
      >
        <header className="register-header">
          ...
          <h2 className="register-header__title">Seller Registration</h2>
          <p className="register-header__subtitle">
            Start selling on the ShopFlow marketplace
          </p>
        </header>
        ...
        <form className="register-form" onSubmit={handleSubmit}>
          ...
          <div
            className={`register-field ${
              fieldErrors.storeName ? 'register-field--error' : ''
            }`}
          >
            <input
              id="reg-storename"
            ...
            <Store
              ...
              className="register-field__icon"
            />
            <label htmlFor="reg-storename" className="register-field__label">
              Store / Brand Name
            </label>
            {fieldErrors.storeName && (
              <span className="register-field__hint">{fieldErrors.storeName}</span>
            )}
          </div>
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? 'Registering seller account...' : 'Register as Seller'}
          </button>
        </form>

        <div className="register-footer">
          Already have a seller account?{' '}
          <Link to="/seller/login" className="register-footer__link">
            Seller Sign In
          </Link>
        </div>
      </motion.div>
